...
def getCosSim(SES_P1, SES_P2, interval=5):
    ...
    for i in range(0, len(a_list), interval):
        ...
        # dev1.append(np.array(a_list[i:i+interval]))
        # dev2.append(np.array(b_list[i:i+interval]))
...
def fit_model_k_fold(X, y):
    ...
    params = {'max_depth':range(100,101), 'criterion':np.array(['entropy','gini'])}

    # Transform 'accuracy_score' into a scoring function using 'make_scorer' 
    scoring_fnc = make_scorer(accuracy_score)

    # Create the grid search object
    grid = GridSearchCV(dtc, param_grid=params,scoring=scoring_fnc,cv=k_fold,error_score='raise')
...
if __name__ == '__main__':
    ...
    # # ===========1211================
    TrainData=mergeAllSnifferData1211()
